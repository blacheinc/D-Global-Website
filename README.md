...
> Note: The shipped font bundle includes `AtypDisplay-Regular.ttf` but no `AtypText-Regular.ttf`. In `app/layout.tsx`, `AtypText-Medium` is aliased to weight 400 so body text resolves. Drop `AtypText-Regular.ttf` into `/fonts` when it's delivered and update the `next/font/local` mapping.
